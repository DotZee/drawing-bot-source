...
def neighbours(pt):
    ...
    for t in swtchr.values():
            pt1 = (pt[0] + t[0], pt[1] + t[1])
            if pt1 in pix:
                n += 1

    return n

def getstart(thresh = 0):
...
def impath(im, showpath = True):
    ...
    for x in range(im.size[0]):
        for y in range(im.size[1]):
            if avg(pixels[x,y]) ==  0: #CHANGE THIS FOR CHANING COLOUR
...
def show(im, p):
    ...
    for k in path:
        for c in k:
            ...
            cv2.imshow('A', cv2.resize(imnew, None, fx = r, fy = r))
        #print k
